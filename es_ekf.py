...
#pt2
var_imu_fs = [21]
var_imu_ws = [0.01,0.009]
var_gnsss  = [0.0000003,0.00000002]
var_lidars = [6,5]

min_err = 100000
minis = [0, 0, 0, 0]

for var_imu_f in var_imu_fs:
    for var_imu_w in var_imu_ws:
        for var_gnss in var_gnsss:           
            for var_lidar in var_lidars:
                ...
                def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
                    ...
                    inverse = inv(h_jac.dot(p_cov_check).dot(h_jac.T) + sensor_var * np.eye(3))
                    ...
                    q_hat = Quaternion(euler = delta_xk[6:]).quat_mult_left(q_check) #proveri je l left ili right
                ...
                for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
                    ...
                    p_check = p_est[k-1] + v_est[k-1] * delta_t + 0.5*(C_ns.dot(imu_f.data[k-1]) + g) * delta_t**2  #proveri je l ide plus ili minus g
                    v_check = v_est[k-1] + (C_ns.dot(imu_f.data[k-1]) + g) * delta_t
                    ...
                    Q = np.eye(6)
                    Q[0:3,0:3] = np.diag([var_imu_f]*3)
                    Q[3:6,3:6] = np.diag([var_imu_w]*3)
                    Q = Q.dot(delta_t**2)
                    ...
                    if gnss_i < len(gnss.t):
                        if (np.abs(imu_f.t[k] - gnss.t[gnss_i]) < 0.01):
                            #print('Updating with gnss')
                            p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check, gnss.data[gnss_i], p_check, v_check, q_check) #proveri 
                            gnss_i+= 1
                    # if lidar_i <len(lidar.t):
                    #     if (np.abs(imu_f.t[k] - lidar.t[lidar_i]) < 0.01):
                    #         #print('Updating with lidar')
                    #         p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check, lidar.data[lidar_i],  p_check, v_check, q_check) #proveri i mozda iskljuci 1 za part 1
                    #         lidar_i+= 1
                ...
                er = (p_est[:8000,0]-gt.p[:8000,0])**2+(p_est[:8000,1]-gt.p[:8000,1])**2+ (p_est[:8000,2]-gt.p[:8000,2])**2
                er = np.sum(er)/8000
                if er < min_err:
                    min_err = er
                    minis = [var_imu_f, var_imu_w, var_gnss, var_lidar]
                    print(minis)
                    print(min_err)

var_imu_f, var_imu_w, var_gnss, var_lidar = minis
print(minis)
